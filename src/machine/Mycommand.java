...
public class Mycommand {
		...
		try {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					Constants.serialPort.setDTR(false);
	...
	public static void sendData(writeFormat w, int slp) {
		Thread t = new Thread(new SerialWriter(Constants.out, w, slp));
		try {
			t.start();
		} catch (Exception e) {
	...
	public static void sendAdcEnableBits(char ch1, char ch2, char ch3, char ch4, char ch5, char ch6, char ch7, char ch8,
			char ch9, char ch10, char ch11, char ch12, char ch13, char ch14, char ch15, int delay) {
		writeFormat wr = new writeFormat();
		wr.addChar('F');
		wr.addChar('M');
		wr.addChar('A');
